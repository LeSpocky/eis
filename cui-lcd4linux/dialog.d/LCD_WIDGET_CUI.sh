...
function dlg_setup_hook()
{
    ...
    if cui_label_new "${dlg}" "Type:"   1 1 13 1 ${IDC_LABEL__WIDGETTYPE} ${CWS_NONE} ${CWS_NONE} ; then
        cui_window_create     "$p2"
    fi

    if cui_listbox_new "${dlg}" "" 1 3 21 5 ${IDC_LISTBOX__WIDGETTYPE} ${CWS_NONE} ${CWS_BORDER} ; then
        ctrl="$p2"
        cui_listbox_callback  "$ctrl" "$LISTBOX_CHANGED" "${dlg}" listbox_changed
        cui_window_setcolors  "$ctrl" "MENU"
        cui_window_create     "$ctrl"
        for currentWidgetType in ${widgetTypes} ; do
            cui_listbox_add   "$ctrl" "$currentWidgetType"
        done
        cui_listbox_select    "$ctrl" "$widgetType"
    fi

    if cui_label_new "${dlg}" "Name:"   24 1 13 1 $IDC_LABEL__WIDGETNAME $CWS_NONE $CWS_NONE ; then
        cui_window_create     "$p2"
    fi

    if cui_listbox_new "${dlg}" "" 24 3 21 5 $IDC_LISTBOX__WIDGETNAME $CWS_NONE $CWS_BORDER ; then
        ctrl="$p2"
        cui_window_setcolors  "$ctrl" "MENU"
        cui_window_create     "$ctrl"
        if [ -f /etc/config.d/cui-lcd4linux-widgets ] ; then
            . /etc/config.d/cui-lcd4linux-widgets
            idx=1
            separator=''
