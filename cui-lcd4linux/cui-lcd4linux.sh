...
addVariables () {
    ...
    if ${added} ; then
        echo ' -> Read documentation for correct parameter usage!'
    fi
}
...
checkIconWidgets () {
    if [ $activeIconWidgets -gt 8 ] ; then
        echo "You have too many active icon widgets! Limiting them to 8"
        activeIconWidgets=8
    fi
}
...
writeLCDConfig () {
    ...
    if [ "$LCD_DRIVER" == "$LCD_MODEL" ] ; then
        echo "   Writing configuration for lcd '$LCD_DRIVER'"
    else
        echo "   Writing configuration for lcd '$LCD_DRIVER', type '$LCD_MODEL'"
    fi
...
writeShutdownConfig () {
    if [ "$LCD_USE_SHUTDOWN_LAYOUT" == 'yes' ] ; then
        ...
        if [ "$LCD_DRIVER" == "$LCD_MODEL" ] ; then
            echo "   Writing shutdown configuration for lcd '$LCD_DRIVER'"
        else
            echo "   Writing shutdown configuration for lcd '$LCD_DRIVER', type '$LCD_MODEL'"
        fi
...
addCronjob () {
	if [ "$LCD_LAYOUT_CYCLE" == 'yes' ] ; then
	    [ ! -d ${crontabPath} ] && mkdir -p ${crontabPath}
	    fi
	    (
	        echo "# =============================================================="
