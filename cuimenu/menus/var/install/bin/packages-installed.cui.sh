...
function load_data()
{
    ...
    then
        ...
        if [ -z $keyword ]
        then
            pm_installed_tolist "$list" "$section"
            cui_window_settext     "$list" ""
        else
            pm_installed_tolist "$list" "$section" "$keyword"
...
#----------------------------------------------------------------------------
# listview_changed_hook
# listitem has been changed
...
function listview_clicked_hook()
{
    ...
    then
        ...
        cui_menu_additem      "$menu" "View upgradable only"    2
        cui_menu_additem      "$menu" "Install/Update package"  3
        cui_menu_additem      "$menu" "Uninstall package"       4
        ...
        then
            ...
            1)
                ...
                show_installed_package_info $win
                ;;
            2)
                cui_window_destroy  "$menu"
                keyword="-u"
...
function init()
{
    ...
    then
        ...
        cui_window_settext        "$mainwin" "Installed packages"
        cui_window_setlstatustext "$mainwin" "Commands: F2=Latest F3=Search F4=View F7=Upgrade F8=Uninstall F10=Exit"
