#!/bin/bash
#------------------------------------------------------------------------------
# /var/install/bin/lvmman.cui.sh - eisfair lvm mananger
#
...
current_module="pvs"             # current (first) module
modules="empty pvs vgs lvs"      # buildin modules
...
function read_modules()
{
    ...
    . $programdir/lvmman.cui.pvs.module.sh
    cui_menu_additem      "$ctrl" "${pvs_menu}" "1"
    . $programdir/lvmman.cui.vgs.module.sh
    cui_menu_additem      "$ctrl" "${vgs_menu}" "2"
    . $programdir/lvmman.cui.lvs.module.sh
    cui_menu_additem      "$ctrl" "${lvs_menu}" "3"
...
function init()
{
    ...
    then
        ...
        cui_window_settext        "$mainwin" "eisfair LVM manager"
