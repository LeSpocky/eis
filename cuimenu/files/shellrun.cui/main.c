...
static TCHAR* ScriptFile = NULL;
static TCHAR* ScriptArgs = NULL;
static TCHAR* Command = NULL;
...
main(int argc, char* argv[])
{
	...
	if (BackendCreatePipes())
	{
		if (ScriptArgs)
		{
			int len = tcslen(ScriptFile) + tcslen(ScriptArgs) + 48;
			
			Command = (TCHAR*) malloc((len + 1) * sizeof(TCHAR));
#ifdef _UNICODE
			stprintf(Command, len, _T("%ls CUISHELL %i %ls"), ScriptFile, getpid(), ScriptArgs);
#else
			stprintf(Command, len, _T("%s CUISHELL %i %s"), ScriptFile, getpid(), ScriptArgs);
#endif
		}
		else
		{
			int len = tcslen(ScriptFile) + 48;

			Command = (TCHAR*) malloc((len + 1) * sizeof(TCHAR));
#ifdef _UNICODE
			stprintf(Command, len, _T("%ls CUISHELL %i"), ScriptFile, getpid());
#else
			stprintf(Command, len, _T("%s CUISHELL %i"), ScriptFile, getpid());
#endif
		}
